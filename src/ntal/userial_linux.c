/****************************************************************************
**
**  Name:       userial_linux.c
**
**  Function:   this file contains functions to interface to transport driver
**              using a gerneric Serial port APIs
**
**
...
static char spi_negotiation[10] = { 0xF0, /* CMD */
 ...
 */
#define READ_LIMIT (GKI_BUF0_SIZE-BT_HDR_SIZE)
/*
...
UINT32 userial_read_thread(UINT32 arg)
{
    ...
    for (;linux_cb.sock > 0;)
    {
        ...
        /* NCI_BUF_POOL_ID: is normally pool 0 */
        if ((p_buf = (BT_HDR *) GKI_getpoolbuf( NCI_BUF_POOL_ID ) )!= NULL)
        {
            p_buf->offset = 0;
        ...
        if (rx_length > 0)
        {
            ...
            GKI_send_event(HCISU_TASK, HCISU_EVT);
        }
        else
        {
            GKI_freebuf( p_buf );
...
BOOLEAN userial_to_tcio_baud(UINT8 cfg_baud, UINT32 * baud)
{
    ...
    {
        APPL_TRACE_ERROR1( "USERIAL_Open: unsupported baud idx %i", cfg_baud );
        *baud = B115200;
        return FALSE;
    }
    return TRUE;
...
UDRV_API void USERIAL_Open(tUSERIAL_PORT port, tUSERIAL_OPEN_CFG *p_cfg, tUSERIAL_CBACK *p_cback)
{
...

    {
        ...
        if ((linux_cb.sock = open((char*)device_name, O_RDWR | O_NOCTTY )) == -1)
        {
            ...
            GKI_send_event(NFC_TASK, NCI_TASK_EVT_TERMINATE);
            return;
        }
        ALOGD( "sock = %d\n", linux_cb.sock);
    ...
    GKI_create_task ((TASKPTR)userial_read_thread, HCISU_TASK, (INT8*)"USERIAL_TASK", 0, 0, (pthread_cond_t*)NULL, NULL);
...
UDRV_API void    USERIAL_Close(tUSERIAL_PORT port)
{
    BT_HDR                  *p_buf = NULL;
    int result;
    ALOGD( "USERIAL_Close(): closing transport (%d)\n", linux_cb.sock);

    if (linux_cb.sock <= 0)
    {
        ALOGD( "USERIAL_Close(): already closed (%d)\n", linux_cb.sock);
        return;
    }
    ...
    if ( result < 0 )
        ...
        ALOGE( "pthread_join() joined: result: %d", result );
    ...
    {
        GKI_freebuf(p_buf);
        ALOGD("USERIAL_Close: dequeued buffer from Userial_in_q\n");
    }
}
