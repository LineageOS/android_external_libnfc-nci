...
**  Name:       gki_ulinux.c
**
**  Function:   this file contains constant GKI for uLinux
**
**
...
UINT16 GKI_wait (UINT16 flag, UINT32 timeout)
{
    ...
    if (p_pthread_info->pCond != NULL && p_pthread_info->pMutex != NULL) {
        int ret;
        LogMsg_3(TRACE_CTRL_GENERAL | TRACE_LAYER_GKI | TRACE_ORG_GKI | TRACE_TYPE_GENERIC,
            "GKI_wait task=%i, pCond/pMutex = %x/%x\n", rtask, p_pthread_info->pCond, p_pthread_info->pMutex);
